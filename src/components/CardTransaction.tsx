                        ...
                        {editingId === it.id ? (
                            <div className="grid gap-2 border border-gray-200 rounded-md p-2">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    <div className="col-span-1">
                                        ...
                                        <AmountInput value={draft.amount} onValue={(v)=>setDraft(p=>({...p, amount: v}))} className="w-full border border-gray-200 rounded-md px-2 py-1 text-[10px]" />
                                    </div>
                                    <div className="col-span-1">
                                        <label className="text-[9px] text-gray-500">Account</label>
                                        <select value={draft.account} onChange={e=>setDraft(p=>({...p, account: e.target.value as any}))} className="w-full border border-gray-200 rounded-md px-2 py-1 text-[10px]">
                                    ...
                                    <div className="col-span-1">
                                        ...
                                        <select value={draft.category} onChange={e=>setDraft(p=>({...p, category: e.target.value}))} className="w-full border border-gray-200 rounded-md px-2 py-1 text-[10px]">
                                ...
                                <div>
                                    ...
                                    <textarea value={draft.description || ''} onChange={(e)=>setDraft(p=>({...p, description: e.target.value}))} className="w-full border border-gray-200 rounded-md px-2 py-1 text-[10px] min-h-14" />
                                </div>
                                <div className="flex items-center justify-end gap-3">
                                    <button className="text-[9px] text-green-600" onClick={()=>saveEdit(it.id, it.type)}>Save</button>
                                    <button className="text-[9px] text-gray-500" onClick={cancelEdit}>Cancel</button>
                                    <button className="text-[9px] text-red-500" onClick={()=>removeTx(it.id)}>Delete</button>
                                </div>
                            </div>
                        ) : (
                            <div className="flex items-center">
                                <div className="flex flex-col flex-1 gap-1">
                                    ...
                                    <div className="flex items-center">
                                        <button className="text-[9px] text-blue-500" onClick={()=>startEdit(it)}>Edit</button>
                                        <button className="text-[9px] text-red-500 ml-3" onClick={()=>removeTx(it.id)}>Delete</button>
                                    </div>
                                </div>
