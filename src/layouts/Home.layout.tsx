...
const HomeLayout = () => {
    ...
    const [filter, setFilter] = useState<FilterState>({ type: 'all', preset: 'all', from: null, to: null });
    ...
    const resetFilter = () => setFilter({ type: 'all', preset: 'all', from: null, to: null });
                ...
                <div className="home-history__wrapper px-2 py-5">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <div className="text-[9px] text-gray-400">{filter.type !== 'all' ? `Filtered: ${filter.type}` : 'All transactions'} {filter.preset !== 'all' ? ` • ${filter.preset}` : ''}</div>
                        </div>
                        ...
                        <div className="relative z-10" ref={dropdownRef}>
                            ...
                            <div className={`origin-top-right absolute right-0 mt-2 w-64 bg-white border border-gray-100 rounded-lg shadow-lg transition-all duration-200 ${isFilterOpen ? 'opacity-100 scale-100 visible' : 'opacity-0 scale-95 invisible'}`} style={{ transformOrigin: 'top right' }}>
                                <div className="p-3 grid gap-2">
                                    ...
                                    <div className="flex gap-2">
                                        <button onClick={() => setFilter(f => ({ ...f, preset: 'all', from: null, to: null }))} className={`flex-1 text-[10px] py-1 rounded ${filter.preset === 'all' ? 'bg-gray-200' : 'bg-gray-50'}`}>All</button>
                                        <button onClick={() => setFilter(f => ({ ...f, preset: 'week', from: null, to: null }))} className={`flex-1 text-[10px] py-1 rounded ${filter.preset === 'week' ? 'bg-gray-200' : 'bg-gray-50'}`}>Per Minggu</button>
                                        <button onClick={() => setFilter(f => ({ ...f, preset: 'month', from: null, to: null }))} className={`flex-1 text-[10px] py-1 rounded ${filter.preset === 'month' ? 'bg-gray-200' : 'bg-gray-50'}`}>Per Bulan</button>
                                    </div>

                                    <div className="text-[10px] font-semibold pt-2">Range</div>
                                    <div className="grid grid-cols-2 gap-2">
                                        <input type="date" value={filter.from ?? ''} onChange={e => setFilter(f => ({ ...f, preset: 'range', from: e.target.value }))} className="text-[10px] border border-gray-100 rounded px-2 py-1" />
                                        <input type="date" value={filter.to ?? ''} onChange={e => setFilter(f => ({ ...f, preset: 'range', to: e.target.value }))} className="text-[10px] border border-gray-100 rounded px-2 py-1" />
                                    </div>
